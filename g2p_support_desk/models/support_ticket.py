...
class SupportTicket(models.Model):
    ...
    team_id = fields.Many2one(
        "support.team", string="Team", tracking=True, default=lambda self: self._get_default_team()
    )
    program_id = fields.Many2one(
        "g2p.program", string="Program", tracking=True, default=lambda self: self._get_default_program()
    )
    ...
    program_beneficiary_ids = fields.Many2many(
        "res.partner", string="Program Beneficiaries", compute="_compute_program_beneficiaries", store=True
    )
    ...
    def _get_default_stage(self):
        try:
            ...
            raise ValidationError(_("Error getting default stage: %s") % str(e)) from e
        ...
        for ticket in self:
            if ticket.partner_id and ticket.program_id:
                membership = self.env["g2p.program_membership"].search(
                    [("program_id", "=", ticket.program_id.id), ("partner_id", "=", ticket.partner_id.id)],
                    limit=1,
                )
                if not membership:
    ...
    def write(self, vals):
        ...
        if "program_id" in vals:
            ...
            if self.partner_id:
                membership = self.env["g2p.program_membership"].search(
                    [("program_id", "=", vals["program_id"]), ("partner_id", "=", self.partner_id.id)],
                    limit=1,
                )
                if not membership:
