...
class CycleCreationWizard(models.TransientModel):
    ...
    sorting_criteria_ids = fields.One2many(
        "cycle.creation.wizard.criteria",
        "wizard_id",
    ...
    def action_confirm(self):
        if self.cycle_id:
            ...
            to_remove = cycle_sorting_criteria.filtered(
                lambda c: c.field_name.id not in new_field_names
            )
            if to_remove:
                to_remove.unlink()
        
            self.cycle_id.write({
                "inclusion_limit": self.inclusion_limit,
